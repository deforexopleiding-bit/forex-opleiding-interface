...
/**
 * PURE render-functie: bouwt de inloggegevens-welkomstmail. Geen send, geen DB,
 * geen throw op ontbrekende velden (best-effort met lege waarden). Zo kan de
 * E-mails-tab (api/email-overzicht.js) er een echte HTML-preview van renderen
 * met voorbeelddata, exact zoals sendCredentialsEmail hem verstuurt.
 *
 * @param {object} opts
 * @param {object} opts.customer     - customer-row (first_name, email, ...)
 * @param {string} opts.tempPassword - tijdelijk wachtwoord
 * @param {string} [opts.loginUrl]   - default uit env BUBBLE_LOGIN_URL
 * @param {string} [opts.lmsLoginUrl] - inloglink nieuw LMS (samen met lmsPassword)
 * @param {string} [opts.lmsPassword] - LMS-wachtwoord; aanwezig → tweede kaartje
 * @returns {{subject:string, html:string, text:string}}
 */
export function renderCredentialsEmail({ customer, tempPassword, loginUrl, lmsLoginUrl, lmsPassword } = {}) {
  const c = customer || {};
  const url = (typeof loginUrl === 'string' && loginUrl.trim()) ? loginUrl.trim() : defaultLoginUrl();
  const naam = (c.first_name || '').trim() || 'jij';
  ...
  const lmsBlockHtml = showLms ? `
      <table cellpadding="0" cellspacing="0" border="0" width="100%" style="background:#f0f9ff;border:1px solid #bae6fd;border-radius:8px;margin:18px 0">
        <tr><td style="padding:14px 18px">
          ...
          <div style="font-size:14px;margin-top:4px;font-family:ui-monospace,SFMono-Regular,Menlo,monospace;color:#111827">${escHtml(c.email)}</div>
  ...
  const text = `Hoi ${naam}, je inloggegevens voor De Forex Opleiding:\n\nInloglink: ${url}\nGebruikersnaam: ${c.email}\nTijdelijk wachtwoord: ${tempPassword}\n\nLog direct in en stel je eigen wachtwoord in.${lmsBlockText}\n— De Forex Opleiding`;
  return { subject, html, text };
}

/**
 * Stuur een persoonlijke welkomstmail met inloggegevens. Fail-soft: gooit NOOIT
 * door naar de caller.
 *
 * @param {object} opts
...
export async function sendCredentialsEmail({ onboarding, customer, tempPassword, loginUrl, lmsLoginUrl, lmsPassword }) {
  try {
    ...
    const { subject, html, text } = renderCredentialsEmail({ customer, tempPassword, loginUrl, lmsLoginUrl, lmsPassword });
    ...
    const result = await sendOnboardingMail({ to: customer.email, subject, html, text });
