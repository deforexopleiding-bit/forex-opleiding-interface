...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Build extra context for Simon (fetch live stats)
    let contextStr = '';
    if (name === 'Simon') {
      try {
        ...
        if (statsResp.ok) {
          ...
          contextStr = `\n\nACTUELE DATA (${new Date().toLocaleString('nl-NL')}):\n- Open taken: ${stats.tasks?.total || 0} (urgent: ${stats.tasks?.urgent || 0}, hoog: ${stats.tasks?.high || 0})\n- Onbeantwoorde mails: ${stats.unanswered?.count || 0}`;
        }
      } catch {}
    }
    ...
    ]).catch(e => console.warn('[agent-chat] save:', e.message));
