// api/simone-suggest.js
// POST -> genereer een Simone-suggestie voor een events-WhatsApp-conversatie.
//
// HTTP-laag (auth + RBAC + body-parse). Suggest-logica zit in
// api/_lib/simone-suggest-core.js zodat de webhook 'm later in-process kan
// aanroepen (sibling-pattern, identiek aan joost-suggest).
//
// Twee auth-paden (sibling-pattern):
//   (a) X-Internal-Token header == process.env.INTERNAL_API_TOKEN
//       → system-pad (reactive auto-suggest vanuit inbox-webhook, in scope
//         voor Fase 2 stap 2b). RBAC geskipt; suggestion krijgt
//         requested_by_user_id=NULL en auto_triggered uit body.
//   (b) Bearer-JWT + events.simone.use permission-check (handmatige
//       "Vraag Simone"-knop in events-inbox UI, in scope voor Fase 2 stap 3).
//
// Permission: events.simone.use (strict — geen fallback; admin krijgt automatisch
...
//   403  geen events.simone.use rechten
...
//   503  ANTHROPIC_API_KEY niet geconfigureerd OF Simone gedeactiveerd voor module
//
// Response 200:
//   {
//     suggestion: {
...
export default async function handler(req, res) {
  ...
  try {
    ...
    console.error('[simone-suggest]', e && e.message);
