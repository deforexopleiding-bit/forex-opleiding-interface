...
export default async function handler(req, res) {
  try { return await _handleImpl(req, res); }
    ...
    console.error('[ls-bulk-cancel] TOP-LEVEL CRASH:', e && e.stack || e?.message || e);
    if (!res.headersSent) {
      res.setHeader('Cache-Control', 'no-store');
      res.setHeader('Content-Type', 'application/json');
      return res.status(500).json({ error: 'Cancel kon niet worden uitgevoerd.', detail: e?.message || String(e) });
    }
  }
}
