    ...
    try {
      ...
      inviter.stateChange.addListener((s) => {
        if (s === 'Establishing') {
          ...
          // wíj af voordat er werd opgenomen, dan is dat een uitspraak over
          // ons, en die hoort niet als 'niet opgenomen' in de cijfers te
          // belanden: zulke rijen telden mee in de pogingenteller, in het
          // dagdoel en in de archiveerregel.
          //
          // GEEN DREMPEL OP DUUR. Bij drie van de negen korte calls in de
          // historie volgt binnen minuten een echt gesprek; een grens op
          // seconden zou dat herbelgedrag afpakken. Het onderscheid komt uit
          // de SIP-staat, niet uit een getal achteraf.
          ...
          _postCallLog(new Date().toISOString(), uitkomst.outcome);
        }
      });
