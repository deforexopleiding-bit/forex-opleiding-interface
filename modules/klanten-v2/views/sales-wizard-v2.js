...
(function () {
  ...
  window.__swSubmitDeal = async (syncToTl) => {
    ...
    try {
      ...
      try { await tryPost('drafts-del', '/api/sales-wizard-drafts', null, 'DELETE'); } catch (_) {}
      // Redirect naar v2 offerte-detail IN-SHELL (batch2b) — was v1 offerte-
      // detail.html. In-shell via __odvOpen zorgt dat sidebar+topbar+tabs
      // blijven staan en de detail rendert in .content. Fallback op standalone
      ...
      if (data?.deal_id) {
        setTimeout(() => {
          const id = String(data.deal_id);
          if (typeof window.__odvOpen === 'function') { window.__odvOpen(id); return; }
          window.location.href = '/modules/offerte-detail-v2.html?id=' + encodeURIComponent(id);
        }, 700);
      }
    } catch (e) {
