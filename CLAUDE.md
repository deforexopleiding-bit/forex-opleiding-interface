...
- **Supabase SQL-editor knipt input op statement-grenzen** (elk statement draait in een eigen transactie). Migraties die daar gedraaid worden mogen daarom GEEN `CREATE TEMP TABLE ... ON COMMIT DROP` gebruiken (de tabel is weg vóór het volgende statement 'em nodig heeft) en GEEN meerdere `DO $$ ... $$;` blocks waarbij het ene block state uit een ander verwacht. Alles wat pre/post-vergelijking nodig heeft: één enkel `DO`-block met de PRE-state in een lokale `jsonb`/variabele. Alles zonder pre/post: losse `UPDATE` / `SELECT` / `ALTER TABLE` statements zonder wrapper. De eerste versie van 2026-07-17-joost-intent-namen-consolideren.sql draaide gedeeltelijk en overschreef productieconfig (escalation_needed disabled → draft, general_question verloor confidence_threshold 0.75). Zie [docs/sql-migrations/archived/](docs/sql-migrations/archived/) voor het afgevoerde bestand met de waarschuwing bovenaan.
- **Kolom-migraties zijn BLOKKEREND als de code de kolom bij naam noemt.** Zodra een INSERT/UPDATE de nieuwe kolom expliciet in het column-list zet (ook al is 'ie nullable), faalt de HELE query met `column "x" does not exist` totdat de migratie is gedraaid. "Nullable dus optioneel voor bestaande rijen" geldt WEL voor rijen die er al waren, NIET voor nieuwe writes die de kolom noemen. Regels voor PR's die een kolom toevoegen én ernaar schrijven:
  1. In de PR-body prominent (bovenaan of in een "⚠ Migratie blokkerend"-blok) vermelden dat de migratie VÓÓR of DIRECT NA de merge moet draaien.
  2. Expliciet noemen wat er stukgaat als 'ie uitblijft (bv. "elke nieuwe Joost-suggestie faalt met column-error → Joost werkt niet meer").
  3. NIET claimen "code werkt ook zonder migratie" tenzij de code de kolom conditioneel meestuurt (via een aparte code-path die de kolom weglaat vóór migratie-run).
  Fouten uit de praktijk: #789 (`proposal_termijnen/uitstel_dagen/termijn_bedrag_eur` op joost_suggestions) en #801 (`promised_date_raw/promised_date_hint`). Beide inserts noemen de kolommen bij naam; zonder migratie was Joost stuk geweest.
