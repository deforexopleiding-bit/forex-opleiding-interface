...
-- ============================================================================
-- ✅ REEDS LIVE GEDRAAID op 2026-08-04 (handmatig in Supabase). Dit bestand
--    documenteert wat er in productie staat; het is administratie/naslag, geen
--    to-do. Idempotent → veilig te herhalen op een andere omgeving.
--
-- ⚠ SUPABASE-EDITOR: draai dit NIET als één groot bestand. De SQL-editor knipt
--    hele bestanden verkeerd op (splitst binnen DO $$-blokken / CREATE-bodies).
--    Draai per blok: elk ALTER / CREATE OR REPLACE / DO $$…END $$ / INSERT los
--    plakken en uitvoeren. Zo is het op 2026-08-04 ook gedraaid.
--
-- Volgorde is bewust: kolommen (3A) VÓÓR upsert_lead (die verwijst ernaar);
-- view-update NA de kolommen. Gedraaid NA de dedup-migratie (leads_email_uniek +
-- upsert_lead bestonden al live).
-- ============================================================================
