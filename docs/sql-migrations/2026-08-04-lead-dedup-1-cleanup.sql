...
--   - aangemaakt = oudste van de groep (eerste-gezien-datum behouden)
...
--
-- UITVOERING: stap 1-4 zitten in één DO $$-blok = één atomaire transactie. De
-- temp table leeft voor het hele blok (ON COMMIT DROP ruimt 'm op bij het einde).
-- GEEN losse BEGIN;/COMMIT; nodig — dat brak in de Supabase-editor door autocommit
-- per statement. STAP 0 en STAP 5 zijn losse controle-SELECT's ervoor/erna.
...
-- STAP 1-4 — ÉÉN atomaire transactie (DO $$-blok).
DO $$
BEGIN
  -- Defensief: eventueel achtergebleven sessie-temp-table opruimen.
  DROP TABLE IF EXISTS lead_survivor;
...
-- STAP 5 — CONTROLE (na het DO-blok): moet 0 rijen geven.
